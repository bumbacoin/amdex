	...
	multisendDialog(0)
{
    updateStyle();
    resize(1000, 600);
    setWindowTitle(tr("Amsterdex [AMDEX]") + " - " + (QString::fromStdString(FormatFullVersion()).split("-")[0]));
	QFontDatabase fontData;
	fontData.addApplicationFont(":/fonts/amdexfont");
#ifndef Q_OS_MAC
	...
	centralWidget->addWidget(posStats);
...
void BitcoinGUI::createActions()
{
    ...
    overviewAction = new QAction(QIcon(":/icons/overview2"), tr("&Dashboard"), this);
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Amsterdex address"));
    ...
    posAction = new QAction(QIcon(":/icons/posstats"), tr("&Statistics"), this);
    posAction->setToolTip(tr("View Proof of Stake statistics"));
    posAction->setCheckable(true);
	posAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(posAction);	
    ...
    aboutAction = new QAction(QIcon(":/icons/toolbar2"), tr("&About Amsterdex"), this);
    aboutAction->setStatusTip(tr("Show information about Amsterdex"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/icons/qt"), tr("About &Qt"), this);
    aboutQtAction->setStatusTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setStatusTip(tr("Modify configuration options for Amsterdex"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/toolbar2"), tr("&Show / Hide"), this);
	...
	amdexSharesAction = new QAction(QIcon(":/icons/amdex_shares"), tr("Shares"), this);
    amdexSharesAction->setToolTip(tr("Open the Amsterdex shares info box"));
	...
	resourcesAMDEXAction = new QAction(QIcon(":/icons/toolbar2"), tr("&Amsterdex.com"), this);
	resourcesAMDEXAction->setToolTip(tr("Visit the Official Amsterdex website"));
	resourcesCHAINAction = new QAction(QIcon(":/icons/web-chain"), tr("&Blockchain Explorer"), this);
	resourcesCHAINAction->setToolTip(tr("Visit the Official Amsterdex Blockchain Explorer"));				
	resourcesTWITTERAction = new QAction(QIcon(":/icons/web-twitter"), tr("&Twitter"), this);
	resourcesTWITTERAction->setToolTip(tr("Visit the Official Amsterdex Twitter"));
	...
	connect(posAction, SIGNAL(triggered()), this, SLOT(gotoPosStats()));
    connect(blockAction, SIGNAL(triggered()), this, SLOT(gotoBlockBrowser()));
	connect(stakeReportAction, SIGNAL(triggered()), this, SLOT(stakeReportClicked()));
	connect(amdexSharesAction, SIGNAL(triggered()), this, SLOT(amdexSharesClicked()));
	connect(resourcesAMDEXAction, SIGNAL(triggered()), this, SLOT(resourcesAMDEXClicked()));
...
void BitcoinGUI::createMenuBar()
{
    ...
    resources->addAction(resourcesAMDEXAction);
...
void BitcoinGUI::createToolBars()
{
	...
	QToolBar *toolbar2 = addToolBar(tr("Amsterdex top-logo"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Amsterdex client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Amsterdex client"));
...
void BitcoinGUI::amdexSharesClicked()
{
   static AmdexSharesDialog dlg;
   //dlg.setModel(walletModel);
   dlg.show();
}
void BitcoinGUI::resourcesAMDEXClicked()
{
     QDesktopServices::openUrl(QUrl("https://amsterdex.com"));
}

void BitcoinGUI::resourcesCHAINClicked()
{
     QDesktopServices::openUrl(QUrl("https://chain.amsterdex.com/"));
}

void BitcoinGUI::resourcesTWITTERClicked()
{
     QDesktopServices::openUrl(QUrl("https://twitter.com/amster_dex"));
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("AMDEX Network<br><br>Connections: <b>%n</b>", "", count));
}
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    {
        labelStakingIcon->setToolTip(tr("You are not staking:<br> <b>Your wallet is locked!</b><br><br>Amsterdex Network Weight: <b>%1</b><br><br>MultiSend: <b>%2</b>").arg(nNetworkWeight).arg(fMultiSend ? tr("Active"):tr("Inactive")));
        labelStakingIcon->setEnabled(false);
        labelStakingIcon->setPixmap(QIcon(":/icons/staking_off").pixmap(28,54));
    }
    else if (vNodes.empty())
    {
        labelStakingIcon->setToolTip(tr("You are not staking:<br> <b>Your wallet is offline!</b><br><br>Amsterdex Network Weight: <b>%1</b><br><br>MultiSend: <b>%2</b>").arg(nNetworkWeight).arg(fMultiSend ? tr("Active"):tr("Inactive")));
        labelStakingIcon->setEnabled(false);
        labelStakingIcon->setPixmap(QIcon(":/icons/staking_off").pixmap(28,54));
    }
    else if (IsInitialBlockDownload())
    {
        labelStakingIcon->setToolTip(tr("You are not staking:<br> <b>Your wallet is syncing with the AMDEX network!</b><br><br>AMDEX Network Weight: <b>%1</b><br><br>MultiSend: <b>%2</b>").arg(nNetworkWeight).arg(fMultiSend ? tr("Active"):tr("Inactive")));
        labelStakingIcon->setEnabled(false);
        labelStakingIcon->setPixmap(QIcon(":/icons/staking_off").pixmap(28,54));
    }
    else if (!nWeight)
    {
        labelStakingIcon->setToolTip(tr("You are not staking:<br> <b>You don't have mature coins!</b><br><br>AMDEX Network Weight: <b>%1</b><br><br>MultiSend: <b>%2</b>").arg(nNetworkWeight).arg(fMultiSend ? tr("Active"):tr("Inactive")));
        labelStakingIcon->setEnabled(false);
        labelStakingIcon->setPixmap(QIcon(":/icons/staking_off").pixmap(28,54));
    }
    ...
    {	
		...
		labelStakingIcon->setToolTip(tr("No issues detected:<br> <b>You are staking!</b><br><br>AMDEX Network Weight: <b>%1</b><br><br>My Wallet Weight: <b>%2</b><br><br>Estimated Next Stake: <b>%4</b><br><br>MultiSend: <b>%5</b>").arg(nNetworkWeight).arg(nWeight).arg(text).arg(fMultiSend ? tr("Active"):tr("Inactive")));
		posStats->setStrength(GetStrength(nWeight));
    }
    else
    {
        labelStakingIcon->setToolTip(tr("You are not staking!<br><br>AMDEX Network Weight: <b>%1</b>").arg(nNetworkWeight));
        labelStakingIcon->setEnabled(false);
        labelStakingIcon->setPixmap(QIcon(":/icons/staking_off").pixmap(28,54));
    }
...
void BitcoinGUI::gotoPosStats()
{
    posAction->setChecked(true);
    centralWidget->setCurrentWidget(posStats);
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Amsterdex address or malformed URI parameters."));
    }
...
void BitcoinGUI::setEncryptionStatus(int status)
{
    ...
    case WalletModel::Unencrypted:
        ...
        labelEncryptionIcon->setToolTip(tr("Your wallet is <b>not encrypted</b> and currently <b>unlocked</b>!<br><br>It is recommended to encrypt your wallet for safety purposes."));
...
{
    if (!fUseAmsterdexTheme)
        return;

    QString qssPath = QString::fromStdString( GetDataDir().string() ) + "/amsterdex.qss";
